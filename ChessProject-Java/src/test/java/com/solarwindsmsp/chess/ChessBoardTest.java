...
public class ChessBoardTest extends TestCase {
    ...
    public void testHas_MaxBoardWidth_of_7() {
        assertEquals(7, ChessBoard.MAX_BOARD_COORDINATE);
    }
    ...
    public void testLimits_The_Number_Of_Pawns()
    {
        for (int i = 0; i < 10; i++)
        {
            ...
            int row = i / ChessBoard.BOARD_SIDE;
            testSubject.add(pawn, 6 + row, i % ChessBoard.BOARD_SIDE, PieceColor.BLACK);
            if (row < 1)
            {
                assertEquals(6 + row, pawn.getXCoordinate());
                assertEquals(i % ChessBoard.BOARD_SIDE, pawn.getYCoordinate());
            }
            else
            {
